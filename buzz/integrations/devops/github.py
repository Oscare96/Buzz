...
class GitHubDevOpsProvider(DevOpsProvider):
    ...
    def repository_status(self,repository:str)->dict[str,Any]:
        repository=self._repository(repository); d=self._get(f"/repos/{repository}"); return {"full_name":d.get("full_name"),"default_branch":d.get("default_branch"),"private":d.get("private"),"archived":d.get("archived")}
    ...
    def run_pipeline(self,repository:str,workflow:str,ref:str)->dict[str,Any]:
        repository=self._repository(repository); workflow=quote(workflow.strip(),safe=""); ref=ref.strip()
        if not workflow or not ref: raise ValueError("Workflow and ref are required.")
